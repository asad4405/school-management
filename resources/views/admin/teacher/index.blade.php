...
@section('title') Teacher Lists @endsection
@section('content')
        ...
        <div class="pd-20 card-box mb-30">
            <div class="clearfix">
                ...
                <div class="pull-right">
                    <a href="{{ route('admin.teacher.create') }}" class="btn btn-primary btn-sm scroll-click"><i
            ...
            <div class="pb-20">
                <table class="table data-table stripe hover nowrap">
                    ...
                    <tbody>
                        @foreach ($user_teacher as $value)
                            <tr>
                                <td class="table-plus">{{ $loop->index + 1 }}</td>
                                <td>{{ $value->name }}</td>
                                <td>{{ $value->email }}</td>
                                <td>{{ $value->teacher->designation }}</td>
                                <td>
                                ...
                                <td>
                                    <div class="d-flex align-items-center" style="gap:6px;">
                                        <a href="{{ route('admin.teacher.edit', $value->id) }}" class="p-0 m-0">
                                            <i class="dw dw-edit2 bg-primary p-2 text-white rounded"></i>
                                        </a>

                                        <form action="{{ route('admin.teacher.destroy', $value->id) }}" method="POST"
